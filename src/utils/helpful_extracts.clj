...
(defn- extract-metric-1 [model num]
  (let [numbers (range num)]
    (for [n numbers]
      (-> model (nth n) :metric))))

(defn- extract-metric-2 [model num]
  (let [numbers (range num)]
    (for [n numbers]
      (-> model (nth n) :other-metrices first :metric))))

(defn- extract-metric-3 [model num]
  (let [numbers (range num)]
    (for [n numbers]
...
(defn eval-maps [model num]
  (let [lambdas (extract-params model num)
        ...
        metric-1 (map #(assoc {} :metric-1 %) (extract-metric-1 model num))
        metric-2 (map #(assoc {} :metric-2 %) (extract-metric-2 model num))
        metric-3 (map #(assoc {} :metric-3 %) (extract-metric-3 model num))]
    (map #(conj %1 %2 %3 %4 %5) lambdas comp-time metric-1 metric-2 metric-3)))
